(function ($) {
    ...
    $("#open-fintering").click(function(){
        $("#filter-options").slideToggle(1000);
    });
...
function showReply(replyText, replyId) {
    console.log(replyText,replyId)
    // Set reply text to replyInput
    $('#replyInput').val(replyText);
...
function incrementLike(action,postId) {
    $.get('/' + action + '/like-post/' + postId, function(data, status) {
        if (data['status'] === 1) {
            $(`#${action}${postId}`).html(`<i class="fa fa-heart" style='color:red' aria-hidden="true"></i> ${data['like']}`)
        }
    });
}